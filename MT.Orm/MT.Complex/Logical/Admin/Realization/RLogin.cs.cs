...
{ 
    ...
    public class RLogin : RBase<T_User>, ILogin
    {
        ...
        {
            return GetAllNoCache().Where(p => p.UserName == username && p.IsDelete == false).Count() > 0;

        }
        public T_User SearchForNameorPwd(string username, string userpwd)
        {
            return GetAllNoCache().Where(p => p.UserName == username && p.Password == userpwd && p.IsDelete == false).FirstOrDefault();
        }

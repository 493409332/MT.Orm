...
{
    [ICOConfig("RUserInfo")]
    public class RUserInfo : RepositoryBaseGeneric, IUserInfo
    {
        public RUserInfo()
        ...
        public T_UserInfo GetUserInfo(T_User User)
        {
            ...
            if ( model == null )
            {
                ...
                List<T_RoleNavBtns> t_rolenavbtnsls = GetAllNoCache<T_RoleNavBtns>().Where(p => t_userrolesidls.Contains(p.RoleID)&&true).ToList();
                ...
                {
                    var nav = GetAllNoCache<T_Navigation>().Where(p => p.ID == item && p.IsDelete != true && p.IsVisible == true).ToList().FirstOrDefault();
                    if ( nav != null )
                    {
                ...
                (p) =>
                {
                    var navbtnsls = GetAllNoCache<T_NavButtons>().Where(z => z.NavId == p.ID && z.IsDelete != true).Select(z => z.ButtonId).ToList();
        ...
        public T_UserInfo GetUserInfoByID(int ID)
        {
            ...
            var quer = GetAllNoCache<T_User>().Where(p => p.ID == ID).ToList().FirstOrDefault();
            if ( quer!=null )
            {

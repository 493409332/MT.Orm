...
namespace MT.Orm.Sqlserver
{
    ...
    public sealed class SqlHelper
    {
        ...
        public static void ExecuteSqlTran(ArrayList SQLStringList, string connectionString, CommandType commandType)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    for (int n = 0; n < SQLStringList.Count; n++)
                    {
                        ...
                        {
                            PrepareCommand(cmd, cmd.Connection, tx, commandType, strsql, (SqlParameter[])null, out mustCloseConnection);
        ...
        public static void ExecuteSqlTran(ArrayList SQLStringList, string connectionString, CommandType commandType, SqlTransaction transaction)
        {
            if (transaction == null) throw new ArgumentNullException("transaction");
            if (transaction != null && transaction.Connection == null) throw new ArgumentException("The transaction was rollbacked or commited, please provide an open transaction.", "transaction");
                ...
                try
                {
                    for (int n = 0; n < SQLStringList.Count; n++)
                    {
                        ...
                        if (strsql.Trim().Length > 1)
                        {
                            PrepareCommand(cmd, transaction.Connection, transaction, commandType, strsql, (SqlParameter[])null, out mustCloseConnection);
                            //cmd.CommandText = strsql;
                            cmd.ExecuteNonQuery();
                        }
                    }

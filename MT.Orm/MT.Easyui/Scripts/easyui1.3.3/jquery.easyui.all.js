...
//		setProperties(container);
	...
	function getTabbyguid(container, which, removeit)
	{
	    var tabs = $.data(container, 'tabs').tabs;
	    ...
	    {
	        var tab = tabs[i]; 
	        if (typeof( $.data(container, 'tabs').tabs[i].panel("options").tabguid) != 'undefined' && $.data(container, 'tabs').tabs[i].panel("options").tabguid == which)
	        {
	            if (removeit)
	            {
	                tabs.splice(i, 1);
	...
	function selectTabbyurl(container, which)
	{
	    var opts = $.data(container, 'tabs').options;
	    var tabs = $.data(container, 'tabs').tabs;
	    var selectHis = $.data(container, 'tabs').selectHis;
	    ...
	    var panel = getTabbyguid(container, which); // get the panel to be activated
	    if (!panel) return;
	...
	function existsbyguid(container, which)
	{
	    return getTabbyguid(container, which) != null;
	}
	...
	$.fn.tabs.methods = {
		...
		selectTabbyurl: function (jq, which)
		{
		    return jq.each(function ()
		    {
		        selectTabbyurl(this, which);
		    });
		},
		select: function(jq, which){
			return jq.each(function(){
		...
		}, existsbyguid: function (jq, which)
		{
		    return existsbyguid(jq[0], which);
		},

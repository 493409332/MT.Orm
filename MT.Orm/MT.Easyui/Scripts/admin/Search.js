...
var group = '<tr><td class="filter-first-td"></td><td colspan="4"><table width="98%" class="group" style="border:0px none;"><tr><th colspan="5" align="left"><select class="opsel"><option value="AND" selected="selected">并且</option><option value="OR">或者</option></select><input type="button" value="+ {}" title="Add subgroup" class="add-group"><input type="button" value="+" title="Add rule" class="add-rule ui-add"><input type="button" value="-" title="Delete group" class="del-group"></th></tr></table></td></tr>';
...
var search = {
    go: function (gridId) {
        ...
        var sDialog = top.$.hDialog({
            href: '/Scripts/admin/search.html?n=' + Math.random(),
            title: '高级查询', iconCls: 'icon-folder_magnify', width: 600, height:300,
            cache: false, closable: true,modal:false,
            toolbar: [
               {
                   text: '条件重置', iconCls: 'icon-lightning', handler: function () {
                    ...
                    text: '查看条件', iconCls: 'icon-script_code', handler: function () {
                        top.$.hDialog({
                            ...
                            width: 600, height: 300, showbtn: false, cache: false, title: '条件 SQL 语句 ', modal: false
                        });
                    }
                }

...
require(["avalon", "easyui", "jqueryAjax", "easyui_lang_zh_CN", "XiucaiEasyUIExtensions", "jqueryvalidate", "linq_jquery", "Search", "json2"], function () {

    var formUrl = '/ScriptsLogical/Admin/User/UserForm.html';
    var edit_formUrl = '/Sys/EditUserForm';
    var editpass_formUrl = '/ScriptsLogical/Admin/User/EditPass.html';



    var mylayout = {
        init: function (size) {
            $('#layout').width(size.width - 8).height(size.height - 19).layout();
    ...
    var crud = {
        initData: function (depid) {
            ...
            top.$('#txt_password').val("");
            top.$('#txt_passSalt').val(randomString());
        },
        add: function () {
            var addDialog = top.$.hDialog({
                href: "/ScriptsLogical/Admin/User/UserForm.html" + '?v=' + Math.random(),
                width: 500,
                height: 400,
                title: '新建帐号',
        ...
        update: function () {
            ...
            if (row) {
                var editDialog = top.$.hDialog({
                    href: "/ScriptsLogical/Admin/User/EditUserForm.html" + '?v=' + Math.random(),
                    width: 500,
                    height: 400,
                    title: '修改帐号',
                    iconCls: 'icon-user_add',
        ...
        editPass: function () {
            ...
            if (row) {
                var pDialog = top.$.hDialog({
                    ...
                    submit: function () {
                        if (top.$('#xiucaiForm').form('validate')) {
                            if (top.$('#txt_newpass').val() == top.$('#txt_repass').val()) {
                                //var query = createParam('editpass', row.ID) + '&password=' + top.$('#txt_newpass').val();
                                var quer = top.$('#xiucaiForm').serializeArray();
                                $.ajaxtext("/User/EditPass", { UserID: row.ID, newPassword: top.$('#txt_newpass').val() }, function (d) {
                                    if (d) {
                                        msg.ok('亲，密码修改成功。');
    ...
    function SetRoles() { //设置角色
        var row = mygrid.selectRow();
        var deps;
        $.getJSON("/Department/GetAllNoReplace", function (data) {
            deps = data
        });
      
        if (row) {
            var rDialog = top.$.hDialog({
                href: '/ScriptsLogical/Admin/User/SetRoles.html', width: 600, height: 500, title: '设置帐号角色', iconCls: 'icon-group',
                onLoad: function () {
                    top.$('#rlayout').layout();
                    ...
                    top.$('#allRoles,#selectedRoles').datagrid({
                        ...
                        columns: [[
                             {
                                 title: '部门名称', field: 'DepartmentId', width: 100, formatter: function (val, row) {
                                     if (val ==0) {
                                         return "无";
                                     }
                                     var quer = Enumerable.from(deps).where(function (x) {
                                         return x.ID == val;
                            ...
                            { title: '备注', field: 'Remark', width: 210 ,hidden:true}
    ...
    $(function () {
        ...
        mylayout.init(size);
        //deptree.init();
        autoResize({ dataGrid: '#userGrid', gridType: 'datagrid', callback: mygrid.databind, height: 50, width: 15 });
